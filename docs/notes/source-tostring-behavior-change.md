...
tags: [note, lexi, api, behavior-change]
created: 2026-07-14
priority: low
effort: low
status: closed
...
## Ruling: the new behavior stands

No code change. A type wrapping source text should render as the source text; `"Lexi.Source"` was the default `object.ToString()` leaking through, not a designed contract. Like [[commonpatterns-static-breaks-consumers]] this did not need [[lexi-3-0-0-is-a-breaking-release]] as cover — it is the better behavior regardless of what number goes on the tag. Release-noted, closed.
